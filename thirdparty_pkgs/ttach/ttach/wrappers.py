...
class SegmentationTTAWrapper(nn.Module):
    ...
    def forward(
        ...
        for transformer in self.transforms:
            ...
            input_batch = {'images': augmented_image}
            augmented_output = self.model(input_batch, *args)
            if self.output_key is not None:
                augmented_output = augmented_output[self.output_key]
            if len(augmented_output.shape) == 3:
                augmented_output = augmented_output.unsqueeze(1)
                # expect B, NUM_CLS, W, H
